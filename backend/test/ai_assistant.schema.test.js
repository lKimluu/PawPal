...
test('AI 小助手 schema 應拒絕超過 150 字的訊息', () => {
  const result = aiAssistantMessageSchema.safeParse({ message: '狗'.repeat(151) })

  assert.equal(result.success, false)
  assert.equal(result.error.issues[0]?.message, '問題內容不可超過 150 字')
})

test('AI 小助手 schema 應接受 150 字以內的訊息', () => {
  const result = aiAssistantMessageSchema.safeParse({ message: '狗'.repeat(150) })
