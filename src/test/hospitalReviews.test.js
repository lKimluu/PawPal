...
test('HospitalReviewModal uses BaseModal and disables submit until rating and comment are valid', () => {
  ...
  assert.match(modal, /reviewDisplayTime/)
  assert.match(modal, /review\.updated_at \|\| review\.created_at/)
  assert.doesNotMatch(modal, /formatReviewTime\(review\.created_at\)/)
