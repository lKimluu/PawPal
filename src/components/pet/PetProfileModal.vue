...
function handleConfirmPetPhotoCrop(croppedFile) {
  ...
  editForm.value.photoFile = croppedFile
  isPetPhotoCropModalOpen.value = false
  clearCropSelection()
  photoErrorMessage.value = ''
...
function handleCancelPetPhotoCrop() {
  isPetPhotoCropModalOpen.value = false
  clearCropSelection()
      ...
      >
        ...
        <div class="grid grid-cols-1 auto-rows-max gap-6 overflow-y-auto overflow-x-hidden pr-1 md:min-h-0 md:flex-1 md:auto-rows-auto md:grid-cols-[260px_minmax(0,1fr)] md:items-start md:overflow-hidden md:pr-0">
          ...
          <div class="min-h-0 min-w-0 space-y-1 overflow-visible pt-1 md:overflow-y-auto md:pr-1">
            ...
            <p v-if="photoErrorMessage || errorMessage" class="mt-3 text-sm font-semibold text-red-600">
